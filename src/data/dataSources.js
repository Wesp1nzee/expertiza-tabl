...
const HANDBOOK_2024 = {
  ...
  logic: {
    ...
    resolveLocationRegionKey(selectedRegion) {
      const { LOCATION_COEFFICIENTS, REGION_ALIASES_FOR_LOCATION } = HANDBOOK_2024.data;
    ...
    resolveWallsRegionKey(selectedRegion) {
      const { WALL_MATERIAL_COEFFICIENTS } = HANDBOOK_2024.data;
    ...
    resolveHouseConditionRegionKey(selectedRegion) {
      const { HOUSE_CONDITION_COEFFICIENTS, REGION_ALIASES_FOR_HOUSE_CONDITION } = HANDBOOK_2024.data;
    ...
    getEvalHouseConditionOptions(regionKey, fundKey) {
      const { HOUSE_CONDITION_COEFFICIENTS } = HANDBOOK_2024.data;
    ...
    resolveFlatConditionRegionKey(selectedRegion) {
      const { FLAT_CONDITION_COEFFICIENTS, REGION_ALIASES_FOR_FLAT_CONDITION } = HANDBOOK_2024.data;
    ...
    getEvalFlatConditionOptions(regionKey) {
      const { FLAT_CONDITION_COEFFICIENTS } = HANDBOOK_2024.data;
    ...
    resolveBalconyRegionKey(selectedRegion) {
      const { BALCONY_COEFFICIENTS, REGION_ALIASES_FOR_BALCONY } = HANDBOOK_2024.data;
    ...
    calcBalconyMultiplier(balconyRegionKey, evalHasBalcony, analogHasBalcony) {
      const { BALCONY_COEFFICIENTS } = HANDBOOK_2024.data;
    ...
    resolveFloorRegionKey(selectedRegion) {
      const { FLOOR_COEFFICIENTS, REGION_ALIASES_FOR_FLOORS } = HANDBOOK_2024.data;
    ...
    getEvalFloorOptions(regionKey, fundKey) {
      const { FLOOR_COEFFICIENTS } = HANDBOOK_2024.data;
